using Castle.DynamicProxy.Generators.Emitters.SimpleAST;
using Composante;
using FluentAssertions;
using Moq;
using System;
using System.Drawing;
using Xunit;

namespace ComposanteTest
{
    public class ClasseTestMockRepository
    {
        private readonly MockRepository _mockRepository;
        private readonly Mock<ICollaborateur> _mockCollaborateur;

        private readonly Classe _classe;

        public ClasseTestMockRepository()
        {
            _mockRepository = new MockRepository(MockBehavior.Strict);
            _mockCollaborateur = _mockRepository.Create<ICollaborateur>();
            _classe = new Classe(_mockCollaborateur.Object);
        }

        [Fact]
        public void Methode_1FoisAvecMockRepository_DevraitAppelerMethode1Fois()
        {
            // Arrange
            var valeur = "test";
            var repetition = 1;
            // Act

            _mockCollaborateur.Setup(x => x.Methode(valeur));

            _classe.Methode(valeur, repetition);
            // Asert
            _mockRepository.VerifyAll();
        }

        [Fact]
        public void Methode_FoisAvecMockRepository_DevraitAppelerMethode2Fois()
        {
            // Arrange
            var valeur = "test";
            var repetition = 2;
            // Act

            _mockCollaborateur.Setup(x => x.Methode(valeur));

            _classe.Methode(valeur, repetition);
            // Asert
            _mockRepository.VerifyAll();
        }

        [Fact]
        public void Methode_LanceExceptionAvecMockRepository_DevraitLancerException()
        {
            // Arrange
            var valeur = "test";
            var repetition = 1;
            // Act

            _mockCollaborateur.Setup(x => x.Methode(valeur)).Throws(new Exception());

            Action action = () => _classe.Methode(valeur, repetition);

            // Asert
            action.Should().Throw<Exception>();
            _mockRepository.VerifyAll();
        }

        [Fact]
        public void Methode2_1FoisAvecMockReposiroty_DevraitAppelerMethode1Fois()
        {
            // Arrange
            var valeur = "test";
            var repetition = 1;
            var retourMethode2 = 2;
            var retourAttendu = 2;
            // Act

            _mockCollaborateur.Setup(x => x.Methode2(valeur)).Returns(retourMethode2);

            var retour = _classe.Methode2(valeur, repetition);
            // Asert
            _mockRepository.VerifyAll();

            retour.Should().Be(retourAttendu);
        }

        [Fact]
        public void Methode2_2FoisAvecMockReposiroty_DevraitAppelerMethode2Fois()
        {
            // Arrange
            var valeur = "test";
            var repetition = 2;
            var retourMethode2 = 2;
            var retourAttendu = 4;
            // Act

            _mockCollaborateur.Setup(x => x.Methode2(valeur)).Returns(retourMethode2);

            var retour = _classe.Methode2(valeur, repetition);
            // Asert
            _mockRepository.VerifyAll();
            retour.Should().Be(retourAttendu);
        }

        [Fact]
        public void Methode2_LanceExceptionAvecMockReposiroty_DevraitLancerException()
        {
            // Arrange
            var valeur = "test";
            var repetition = 1;
            // Act

            _mockCollaborateur.Setup(x => x.Methode2(valeur)).Throws(new Exception());

            Action action = () => _classe.Methode2(valeur, repetition);

            // Asert
            action.Should().Throw<Exception>();
            _mockRepository.VerifyAll();
        }

    }
}
